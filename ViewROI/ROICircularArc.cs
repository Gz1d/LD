...
namespace ViewROI
{
	...
	public class ROICircularArc : ROI
	{
		...
		public ROICircularArc(double radiusIn)
		{
			NumHandles = 4;         // midpoint handle + three handles on the arc
			activeHandleIdx = 0;
        ...
        public void GetCircleArc(out double OutCenterRow,out double OutCenterCol,out double OutR,
			out double OutStartPhi,out double OutEndPhi)
		{
			OutCenterRow = this.midR;
			OutCenterCol = this.midC;
			OutR = this.radius;
			OutStartPhi = this.startPhi;
			OutEndPhi = this.startPhi+ this.extentPhi;
		}
		...
		private void updateArrowHandle()
		{
			...
			double headWidth  = 15;
			if (arrowHandleXLD == null) arrowHandleXLD = new HXLDCont();
