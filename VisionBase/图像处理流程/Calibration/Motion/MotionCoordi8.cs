...
{
   public class MotionCoordi8:MotionBase
    {
        public override bool SetCoordiPos(double X, double Y, double Theta)
        {
            bool IsOk = true;
            IsOk = IsOk && SetAixsPos(AxisEnum.Coordi8_X1, X);
            IsOk = IsOk && SetAixsPos(AxisEnum.Coordi8_Y1, Y);
            IsOk = IsOk && SetAixsPos(AxisEnum.Coordi8_Theta1, Theta);
        ...
        public override bool GetCoordiPos(out double X, out double Y,out double Z, out double Theta)
        {
            ...
            Z = 0;
            Theta = 0;
            IsOk = IsOk && GetAxisPos(AxisEnum.Coordi8_X, out X);
            IsOk = IsOk && GetAxisPos(AxisEnum.Coordi8_Y, out Y);
            IsOk = IsOk && GetAxisPos(AxisEnum.Coordi8_Z, out Z);
            IsOk = IsOk && GetAxisPos(AxisEnum.Coordi8_Theta, out Theta);
            return IsOk;
        }
    }
}
